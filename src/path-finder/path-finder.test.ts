...
describe('testing path finder class', () => {
    test('should get correct result for simple single search term', async () => {
        ...
        // to keep path output out of the testing output, so use ts-ignore
        const logSpy = jest
        ...
        const pathFinder = new PathFinder(
            layout,
            createReadStream('test-inputs/test1.txt'),
            process.stdout
        );
    ...
    test('should get correct result for multiple search terms', async () => {
        ...
        const pathFinder = new PathFinder(
            layout,
            createReadStream('test-inputs/test2.txt'),
            process.stdout
        );
        await pathFinder.computeCursorPaths();
    ...
    test('should get correct result for corner keys', async () => {
        ...
        const pathFinder = new PathFinder(
            layout,
            createReadStream('test-inputs/test3.txt'),
            process.stdout
        );
        await pathFinder.computeCursorPaths();
    ...
    test('should get correct result for spaces only', async () => {
        ...
        const pathFinder = new PathFinder(
            layout,
            createReadStream('test-inputs/test4.txt'),
            process.stdout
        );
        await pathFinder.computeCursorPaths();
